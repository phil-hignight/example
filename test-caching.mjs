...
function createTasksStore(projectDir) {
   ...
   * given, only tasks at that tier. Returns lightweight records (no messages).
   */
  async function listTasks({ parentId = undefined, tier = undefined } = {}) {
    const out = [];
   ...
   * not the current task and is finished. Returns its id or null.
   */
  async function findOldestDemotionCandidate(tier) {
    const tasks = await listTasks({ tier });
    const finished = tasks.filter((t) => t.ended != null && t.id !== state.currentId);
    finished.sort((a, b) => (a.ended ?? 0) - (b.ended ?? 0));
    return finished[0]?.id ?? null;
  }

  /** Compute token usage per tier from current task records. */
...
const TIER_1_BUDGET = 12000;     // tokens; current + recent tasks (full)
const TIER_2_BUDGET = 8000;      // tokens; masked-results tasks
const HYST_HIGH = 1.10;          // demote when tier exceeds 110% of budget
const HYST_LOW = 0.90;           // stop demoting when back under 90%
...
async function maybeDemote(store) {
  // Compute tier 1 token usage from all tier-1 tasks excluding the current one
  // (current may temporarily exceed budget — that's allowed).
  const state = store._state();
  let t1Tokens = 0;
  const tier1 = await store.listTasks({ tier: 1 });
  for (const t of tier1) {
    if (t.id === state.currentId) continue;
    const full = await loadTaskFull(store, t.id);
    for (const m of full?.messages ?? []) t1Tokens += estTokens(m.content);
  }
  // Demote tier 1 -> tier 2 while above hysteresis high.
  const t1Limit = TIER_1_BUDGET * HYST_HIGH;
  const t1Stop  = TIER_1_BUDGET * HYST_LOW;
  while (t1Tokens > t1Limit) {
    const candidateId = await store.findOldestDemotionCandidate(1);
    if (!candidateId) break;
    const before = await loadTaskFull(store, candidateId);
    let removed = 0;
    for (const m of before?.messages ?? []) if (m.kind === 'tool-result') removed += estTokens(m.content);
    await store.demoteToTier2(candidateId);
    t1Tokens -= removed;
    if (t1Tokens < t1Stop) break;
  }
  // Demote tier 2 -> tier 3 while above hysteresis high (mechanical: drop messages).
  let t2Tokens = 0;
  const tier2 = await store.listTasks({ tier: 2 });
  for (const t of tier2) {
    const full = await loadTaskFull(store, t.id);
    for (const m of full?.messages ?? []) t2Tokens += estTokens(m.content);
  }
  const t2Limit = TIER_2_BUDGET * HYST_HIGH;
  const t2Stop  = TIER_2_BUDGET * HYST_LOW;
  while (t2Tokens > t2Limit) {
    const candidateId = await store.findOldestDemotionCandidate(2);
    if (!candidateId) break;
    const before = await loadTaskFull(store, candidateId);
    let removed = 0;
    for (const m of before?.messages ?? []) removed += estTokens(m.content);
    await store.demoteToTier3(candidateId, before?.summary || '(no summary)');
    t2Tokens -= removed;
    if (t2Tokens < t2Stop) break;
  }
}
