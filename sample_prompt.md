...
/* A check is work to do; a fact is something to record. The card says which. */
.item-check {
  border-left: 4px solid var(--check);
  background: linear-gradient(90deg, var(--check-tint) 0 260px, var(--panel) 520px);
}
.item-check > .ihead h3 { font-weight: 650; }
.item-issues { border-left: 4px solid var(--warn); }

/* What needs attention stands out. A problem shouts, an undetermined answer speaks up, and
   a check that passed settles back to the resting look with a tick. */
.item.state-finding {
  border-left: 6px solid var(--bad);
  background: linear-gradient(90deg, #fce8e4 0 300px, var(--panel) 640px);
}
.item.state-finding > .ihead h3 { font-weight: 700; }
.item.state-finding > .ihead .kind-check { background: var(--bad); }
.item.state-unable { border-left: 4px solid var(--warn); background: linear-gradient(90deg, #fff6e0 0 220px, var(--panel) 440px); }
.item.state-unable > .ihead .kind-check { background: var(--warn); }
...
@media print {
  ...
  .item-check, .item-issues, .item.state-finding, .item.state-unable { background: none !important; }
  .item.state-finding { border-left-width: 5px !important; }
    ...
    DEF.items.forEach(function (it) {
      ...
      if (it.type === 'check') {
        ...
        if (rs2) {
          ...
          setState(u.root, f ? 'finding' : (unc ? 'unable' : (ok ? 'pass' : 'open')));
        } else {
          ...
          setState(u.root, a.status === 'finding' ? 'finding' : (a.status === 'unable' ? 'unable' : (ok ? 'pass' : 'open')));
        }
      } else {
