...
3. **Clone it outside this project**, so the project's own file list stays as the developers see it, and check out the ref you were given:

```sh
git clone <gitlab url> ../deps/<name>
git -C ../deps/<name> checkout <tag, branch, or commit>
git -C ../deps/<name> log -1 --format=%H%d
```

   If the clone fails, for want of network, credentials, or the right path, say exactly what failed and ask again. Do not work around it.
...
5. **Record and continue.** Put the URL, the ref, and the clone path in the topic 0 item for source outside this project, then go back to the item you stopped at and answer it from the code you now have. Cite files in the clone by that path, for example `../deps/web-security/src/main/java/.../SsoFilter.java:88`; the linter checks those paths exist, so leave the clone in place until the review is finished. Add a row for every repository you had to fetch, as you fetch it.
...
- Where the source is now (clone path), or not-available
...
<!-- answer C-06 -->
| Library, service, or repository | Source needed | Why: what it provides that a check must read, or why not needed | GitLab URL (from the developer) | Ref checked out, verified against the version used | Where the source is now (clone path), or not-available | Source obtained |
|---|---|---|---|---|---|---|
|   |   |   |   |   |   |   |
...
function fileIndex(fs, path, root) {
  ...
  return {
    ...
    resolve: function (p) {
      ...
      if (q.indexOf('../') === 0 || /^[A-Za-z]:\//.test(q) || q.charAt(0) === '/') {
